...
--We want to find a path that a list of plot nodes from given source plot to destination plot
function Route_FindPathByPlot( plot1, plot2 )
	...
	function NodeToString( node )
		...
		local from = Asset_Get( node.route, RouteAssetID.FROM_PLOT )
		local to   = Asset_Get( node.route, RouteAssetID.TO_PLOT )		
	...
	if route then
		InputUtil_Pause( "cache route" )
		--MathUtil_Dump( route )
		--below should be modified.
		local path = InsertPath( route, plot1, plot2 )
		_p2pPaths[id1] = path
